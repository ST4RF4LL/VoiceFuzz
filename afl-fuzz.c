...
EXP_ST u64 total_crashes,             /* Total number of crashes          */
           ...
           wrong_translations;        //Wh4lter.total number of wrong translations
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det,u8* ori_trans,int edit_distance,u8 eff_map_number) {
  ...
  q->orig_translation = ori_trans;
  q->edit_distance = edit_distance;
  // printf("%s\n",q->orig_translation);
  //set eff_map
  q->eff_map_number = eff_map_number;
...
static void read_testcases(void) {
  ...
  for (i = 0; i < nl_cnt; i++) {
    ...
    ori_trans = malloc(strlen(temp)*sizeof(char));
    strcpy(ori_trans,temp);
    // printf("ori_translation:%s\n",ori_trans);
    edit_distance = atof(strtok(NULL,delims));
    // printf("%d\n",edit_distance);

    add_to_queue(fn, st.st_size, passed_det,ori_trans,edit_distance,0);
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (!forksrv_pid) {
    ...
    // int fd = open(FIFO,O_WRONLY|O_NONBLOCK);//bug!
    // if(dup2(fd,1)<0)PFATAL("W dup2() failed");
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault,int edit_distance,u8* eff_map) {
  ...
  if (fault == crash_mode) {
    ...
    add_to_queue(fn, len, 0,orig_trans,edit_distance,0);
  ...
  if (fault == FAULT_TRANS)
  {
    ...
    add_to_queue(fn, len, 0,orig_trans,edit_distance,eff_map_number);
...
keep_as_crash:
    ...
    fprintf(distance_input_f,"%d,",edit_distance);
      ...
      // if (evaluate_testcase(outbuf_distance,edit_distance)>0.5)
      // {
      ...
      fn = alloc_printf("%s/mistakes/id_%06llu_%02u", out_dir, wrong_translations,
                        kill_signal);
...
EXP_ST u8 common_fuzz_stuff(char** argv,u8* in_buf, u8* out_buf, u32 len,int edit_distance,u8* eff_map) {
  ...
  queued_discovered += save_if_interesting(argv, out_buf, len, fault,edit_distance,eff_map);
...
static u8 fuzz_one(char** argv) {
  ...
  //Wh4lter
  // orig_trans = malloc(100*sizeof(char));
  orig_trans = queue_cur->orig_translation;
  // strcpy(orig_trans,queue_cur->orig_translation);
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
  ...
  // eff_map    = ck_alloc(EFF_ALEN(len/2));
    ...
    u8* map_name;
    map_name=alloc_printf("%s/eff_maps/%06u",out_dir,queue_cur->eff_map_number);

    if (!access(map_name,0))
    {
      //try load eff_map
    ...
    for (j = 1; j <= ARITH_MAX; j++) {
      ...
      if ((orig & 0xff) + j > 0xff && !could_be_bitflip(r1) && j < eff_map[i/2]) {
        ...
        *(u16*)(out_buf + i) = orig + j;
        ...
        if (wrong_flag)update_eff(eff_map,i/2,j);
        // {
    ...
    for (j = 0; j < sizeof(interesting_16) / 2; j++) {
      ...
      if ((u16)interesting_16[j] != SWAP16(interesting_16[j]) &&
        ...
        *(u16*)(out_buf + i) = SWAP16(interesting_16[j]);
        ...
        if (wrong_flag)update_eff(eff_map,stage_cur_byte,abs((u16)SWAP16(stage_cur_val)-orig));
        // {
...
//now using ex_eff_map
  ...
  for (i = 0; i < len - 3; i+=2) {
    ...
    *(u32*)(out_buf + i) ^= 0xFFFFFFFF;
    ...
    ex_temp = *(u16*)(out_buf + i) ^ 0xFFFF;
    ex_temp2 = *(u16*)(out_buf + i+1) ^ 0xFFFF;

    if (!(ex_temp<eff_map[i/2])&&!(ex_temp2<eff_map[i/2+1]))
    {
      stage_max--;
      continue;
    }
  ...
  stage_name  = "arith 32/16";
  stage_short = "arith32";
  stage_cur   = 0;
  stage_max   = 2 * (len - 3) * ARITH_MAX;
  ...
  for (i = 0; i < len - 3; i+=2) {
    ...
    for (j = 1; j <= ARITH_MAX; j++) {
      ...
      if ((orig & 0xffff) + j > 0xffff && !could_be_bitflip(r1) && j<eff_map[i/2]) {
        ...
        *(u32*)(out_buf + i) = orig + j;
        ...
        if (common_fuzz_stuff(argv, orig_in,out_buf, len,queue_cur->edit_distance,eff_map)) goto abandon_entry;
        stage_cur++;
        if (wrong_flag)update_eff(eff_map,i/2,j);//little endian
        // if (wrong_flag)update(eff_map,i/2,j);


      } else stage_max--;

      if ((orig & 0xffff) < j && !could_be_bitflip(r2) && j<eff_map[i/2]) {
      ...
      if ((SWAP32(orig) & 0xffff) + j > 0xffff && !could_be_bitflip(r3) && j<eff_map[i/2+1]) {
        ...
        *(u32*)(out_buf + i) = SWAP32(SWAP32(orig) + j);
        ...
        if (common_fuzz_stuff(argv, orig_in,out_buf, len,queue_cur->edit_distance,eff_map)) goto abandon_entry;
        stage_cur++;
        if (wrong_flag)update_eff(eff_map,i/2+1,j);//big endian


      } else stage_max--;

      if ((SWAP32(orig) & 0xffff) < j && !could_be_bitflip(r4) && j>eff_map[i/2]) {
  ...
  stage_name  = "interest 32/16";
  stage_short = "int32";
  stage_cur   = 0;
  stage_max   = (len - 3) * (sizeof(interesting_32) >> 2);
  ...
  for (i = 0; i < len - 3; i+=2) {
    ...
    for (j = 0; j < sizeof(interesting_32) / 4; j++) {
      ...
      ex_temp = interesting_32[j]&0x0000FFFF;
      ex_temp2 = interesting_32[j] >> 16;

      if (!could_be_bitflip(orig ^ (u32)interesting_32[j]) &&
          !could_be_arith(orig, interesting_32[j], 4) &&
          !could_be_interest(orig, interesting_32[j], 4, 0) &&
          (ex_temp<eff_map[i/2] || ex_temp2 < eff_map[i/2+1])) {
        ...
        *(u32*)(out_buf + i) = interesting_32[j];
        ...
        if(wrong_flag)
        {
          update_eff(eff_map,i/2,ex_temp);
          update_eff(eff_map,i/2+1,ex_temp2);
        }
      ...
      if ((u32)interesting_32[j] != SWAP32(interesting_32[j]) &&
          ...
          (SWAP16(ex_temp)<eff_map[i/2] || SWAP16(ex_temp2) < eff_map[i/2+1])) {
        ...
        *(u32*)(out_buf + i) = SWAP32(interesting_32[j]);
        ...
        if(wrong_flag)
        {
          update_eff(eff_map,i/2,SWAP16(ex_temp));
          update_eff(eff_map,i/2+1,SWAP16(ex_temp2));
        }
...
goto havoc_stage;
  ...
  for (i = 0; i < len; i++) {
    ...
    for (j = 1; j <= ARITH_MAX; j++) {
      ...
      if (!could_be_bitflip(r)) {
        ...
        stage_cur++;
        //update eff map
        *(u8*)eff_map = (u8)min(j,*(u8*)eff_map);
  ...
  for (i = 0; i < len; i++) {
    ...
    for (j = 0; j < sizeof(interesting_8); j++) {
      ...
      ex_temp = abs(*(out_buf+i)-interesting_8[j]);
      if (could_be_bitflip(orig ^ (u8)interesting_8[j]) ||
          could_be_arith(orig, (u8)interesting_8[j], 1) ||  ex_temp>*(u8*)(eff_map+i)) {
        stage_max--;
        continue;
      }
...
//
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (eff_map[stage_cur_byte/2] || UR(10)<9)
    {
      stage_max--;
      continue;
    }
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (eff_map[stage_cur_byte/2] || UR(100)<95)
    {
      stage_max--;
      continue;
    }
...
goto skip_user_extras;
  ...
  for (i = 0; i <= len; i++) {
    ...
    for (j = 0; j < extras_cnt; j++) {
      ...
      if (common_fuzz_stuff(argv,orig_in, ex_tmp, len + extras[j].len,queue_cur->edit_distance,eff_map)) {
        ck_free(ex_tmp);
        goto abandon_entry;
      }
      queue_cur->edit_distance-=extras[j].len;
...
//Wh4lter wrong translation mistakes
  ...
  tmp = alloc_printf("%s/eff_maps", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:T:dnCB:S:M:x:QVl:")) > 0)
    ...
    switch (opt) {
      ...
      case 'l':
        if(List_name) FATAL("Multiple -l options not supported");
        List_name = optarg;
        break;
